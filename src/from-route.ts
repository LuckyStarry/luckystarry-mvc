...
export function FromRoute(path: string, paramType?: ParameterType) {
  return function (target: any, propertyKey: string, parameterIndex: number) {
    logger.Info(`${target.constructor.name}.${propertyKey} Parameter:${path} => Index:${parameterIndex}`)
    ...
    builder.Set(parameterIndex, new ArgumentGetterRoute(propertyKey, path, paramType))
  }
}
